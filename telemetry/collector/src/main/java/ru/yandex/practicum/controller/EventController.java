    ...
    public void collectSensorEvent(SensorEventProto request, StreamObserver<Empty> responseObserver) {
        try {
            ...
            if (sensorEventHandlers.containsKey(request.getPayloadCase())) {
                ...
                log.error("Не найден обработчик для события {}", request.getPayloadCase());
                throw new HandlerNotFoundException("Не найден обработчик для события " + request.getPayloadCase());
            }

            responseObserver.onNext(Empty.getDefaultInstance());
            responseObserver.onCompleted();
        } catch (Exception exception) {
            log.error("Ошибка при обработке события {}", request.getPayloadCase(), exception);
